...
$menu = tampil_menu("SELECT * FROM menu");
$id_otomatis = GenerateIdMenu();
$message = '';

if (isset($_GET["submit"])){
    if (tambah_menu($_GET) > 0) {
        echo"
        <script>
...
<head>
    ...
    <title>Tambah Menu F&B - STATIC.</title>
...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-10">
        <div>
            <h1 class="text-4xl font-black tracking-tighter uppercase">NEW <span class="text-amber-500">CATERING</span></h1>
            <p class="text-gray-500 text-xs tracking-[0.4em] uppercase font-semibold">F&B Inventory System</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
        <div class="md:col-span-2 space-y-6">
            <div class="glass p-8 rounded-[2.5rem] border-l-4 border-amber-500/50">
                ...
                <h4 class="text-lg font-bold mb-2 text-white">Registrasi Menu</h4>
                <p class="text-sm text-gray-400 leading-relaxed">
                    Pastikan nama menu menarik dan kategori (Jenis) sesuai. Harga yang diinput akan menjadi harga dasar pada sistem kasir.
                </p>
            </div>
            ...
            <div class="glass p-6 rounded-3xl flex items-center gap-4">
                ...
                <div>
                    <p class="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Auto ID Generated</p>
                    <p class="text-sm font-mono text-white"><?= $id_otomatis; ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="md:col-span-3">
            <div class="glass p-10 rounded-[2.5rem] shadow-2xl">
                ...
                <form action="" method="GET" class="space-y-5">
                    <div>
                        <label class="block text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-2 ml-1">ID Menu</label>
                        <input type="text" name="id_menu" value="<?= $id_otomatis; ?>" class="input-glass w-full px-4 py-3 opacity-60 cursor-not-allowed font-mono" readonly>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" placeholder="e.g. Popcorn Caramel Large" class="input-glass w-full px-4 py-3" required>
                    </div>

                    <div>
                        <label class="block text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-2 ml-1">Harga Satuan</label>
                        <div class="relative">
                            <span class="absolute left-4 top-3 text-amber-500 font-bold">Rp</span>
                            <input type="number" name="harga" placeholder="0" class="input-glass w-full pl-12 pr-4 py-3 text-lg font-bold" required min="0">
                        </div>
                    </div>
                    ...
                    <div class="pt-6">
                        <button type="submit" name="submit" value="Tambah" class="w-full bg-pink-600 hover:bg-pink-700 text-white py-4 rounded-2xl font-black tracking-[0.2em] text-sm transition-all shadow-lg shadow-pink-900/30 flex justify-center items-center gap-3 uppercase">
                            Simpan Menu <i class="fas fa-calendar-check"></i>
                        </button>
                    </div>
                </form>
            </div>
